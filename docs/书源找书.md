...
- URL 后缀 JSON：`method`、`body`、`headers`、`charset`、`webView`、`webJs`、`bodyJs`、`proxy`（`body: '{"k":1}'` 等单引号写法经 `normalizeLegadoLooseJson` 解析；JSON body 默认 `Content-Type: application/json`）
- `&&` / `||` / `%%`；`##` 正则替换（含首替-only 后缀）
