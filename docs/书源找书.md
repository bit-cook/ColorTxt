...
彩读的「找书」功能兼容 [Legado（开源阅读）](https://github.com/gedoor/legado) 文本书源 JSON 格式，
...
| 模板 / 页码 | 整串 `replaceKeyPageJs` **后再**切 UrlOption | 先 `applyTemplateAsync`，再 `splitUrlFetchOptions` |
...
| 请求 | OkHttp（忽略证书）+ WebView | `fetchStrResponse` + dispatcher + `backstageWebView` |
...
- 普通 GET 若为 JS 挑战壳，仅刷 Cookie 再 fetch 在 Electron 里常仍无正文，须走 webView。  
- object body 须 `JSON.stringify`，勿 `String(obj)`→`[object Object]`。  
- JS 挑战 / `probev3` WAF 壳可自动改走 webView（`rejectUnauthorized: false`）。  
...
| `java.refreshBookUrl()` | 把 `book.bookUrl` 写成详情最终 URL；常见于同页目录 |
...
| `loginCheckJs` | `loginCheck.ts` | 请求后检测；自动 await；挑战壳则 webView 拉 HTML |
...
- 生效范围：`fetchStrResponse` / 封面拉取等走 `getBookSourceDispatcher` 的请求；
