...
| macOS | x64（Intel） | `macos-latest`（交叉编译；`prune-pack-deps` 补装 x64 原生 optional 包） | DMG |
...
交叉编译注意：`npm ci` 只会安装**构建机**架构的 optional 原生包。例如在 Apple Silicon runner 上打 **macOS Intel（darwin-x64）** 时，默认没有 `@node-rs/jieba-darwin-x64` / `sqlite-vec-darwin-x64`。`prune-pack-deps` 会在裁剪前用 **`npm pack`** 拉取目标平台包并校验原生二进制；缺失则直接失败，避免打出无法启动的安装包。
